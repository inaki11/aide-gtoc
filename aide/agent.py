...
class Agent:
    ...
    def _draft(self) -> Node | None:
        ...
        # --- FIX: Check if code generation failed ---
        if not code or not code.strip():
            logger.warning("Drafting failed: No code generated by LLM.")
            return None
        # --------------------------------------------
    ...
    def _improve(self, parent_node: Node) -> Node | None:
        introduction = (
        ...
        # --- FIX: Check if code generation failed ---
        if not code or not code.strip():
            logger.warning("Improvement failed: No code generated by LLM.")
            return None
        # --------------------------------------------
    ...
    def _debug(self, parent_node: Node) -> Node | None:
        introduction = (
        ...
        # --- FIX: Check if code generation failed ---
        if not code or not code.strip():
            logger.warning("Debugging failed: No code generated by LLM.")
            return None
        # --------------------------------------------
    ...
    def step(self, exec_callback: ExecCallbackType):
        ...
        # --- FIX: Skip step if generation failed ---
        if result_node is None:
            logger.error("Step aborted: Code generation returned empty result. Skipping execution.")
            return
        # -------------------------------------------
